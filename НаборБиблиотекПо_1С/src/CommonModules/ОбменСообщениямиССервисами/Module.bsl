...
Функция ОтправитьЗапросHTTP(Знач ИмяСервера, Знач АдресРесурса = "", Знач Заголовки = Неопределено, ТекущийМетод = "", Тело = Неопределено,
	...
	Если Не КодОбщегоНазначенияКлиентСерверПовтИсп.ПроверитьВозможностьВыполнения() Тогда
...
Процедура ДобавитьАвторизациюВЗаголовки(Заголовки, АвторизацияЧерез, Перезапросить = Ложь) Экспорт
	...
	Если ВРег(АвторизацияЧерез) = ВРег("Keycloak") Тогда
		Заголовки.Вставить("Authorization", ПолучитьТокенЧерезKeycloak(Перезапросить));
	КонецЕсли;
	
КонецПроцедуры

Функция ПолучитьТокенЧерезKeycloak(ОбновитьТокен = Ложь) Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	Настройка = КодОбщегоНазначения.ПолучитьДополнительнуюНастройкуПрограммы(,, "Авторизация keycloak");
	
	ДатаДействияТокена = КодОбщегоНазначения.ПолучитьЭлементСтруктуры(Настройка, "ДатаДействияТокена");
	Токен = КодОбщегоНазначения.ПолучитьЭлементСтруктуры(Настройка, "Токен");
	ТипТокена = КодОбщегоНазначения.ПолучитьЭлементСтруктуры(Настройка, "ТипТокена");
	
	Если ДатаДействияТокена = Неопределено Или ДатаДействияТокена < (ТекущаяДатаСеанса() + 2) // чтобы начинать обновлять токен заранее
		Или Токен = Неопределено Или ОбновитьТокен Тогда
		ИмяСервера = "keycloak.webbankir.team";
		Логин = "1c";
		Пароль = "68770ea7-a1da-46ac-8fff-e62c64c779ec";
		ИмяМетода = "auth/realms/services/protocol/openid-connect/token";
		...
		Результат = ОбменСообщениямиССервисами.ОтправитьЗапросHTTP(ИмяСервера, ИмяМетода, Заголовки, "keycloak/auth", Тело,, Истина,,,,,,, Логин, Пароль);
		
		Если Результат.КодСостояния > 199 И Результат.КодСостояния < 300 Тогда
			ДатаДействияТокена = ТекущаяДатаСеанса();
			СтруктураОтвета = ОбменСообщениямиССервисами.ПрочитатьJSONвДанные(Результат.ПолучитьТелоКакСтроку(),, Истина);
			ДатаДействияТокена = ДатаДействияТокена + КодОбщегоНазначения.ПолучитьЭлементСтруктуры(СтруктураОтвета, "expires_in", Тип("Число"));
			Токен = КодОбщегоНазначения.ПолучитьЭлементСтруктуры(СтруктураОтвета, "access_token");
			ТипТокена = КодОбщегоНазначения.ПолучитьЭлементСтруктуры(СтруктураОтвета, "token_type");
			
			Попытка
				Если Настройка = Неопределено Тогда
					Настройка = КодОбщегоНазначения.СоздатьПриНеобходимостиДополнительнуюНастройкуПрограммы(
						"Авторизация keycloak", Истина, Новый Структура("Токен, ДатаДействияТокена, ТипТокена", Токен, ДатаДействияТокена, ТипТокена));
				Иначе
					НачатьТранзакцию();
					
					Блокировка = Новый БлокировкаДанных;
					ЭлементБлокировки = Блокировка.Добавить("Справочник.ДополнительныеНастройкиПрограммы");
					ЭлементБлокировки.УстановитьЗначение("Ссылка", Настройка.Ссылка);
					ЭлементБлокировки.Режим = РежимБлокировкиДанных.Разделяемый;
					Блокировка.Заблокировать();
					
					НастройкаОбъект = Настройка.Ссылка.ПолучитьОбъект();
					СтрокаТокен = НастройкаОбъект.Параметры.Найти("Токен", "Имя");
					Если СтрокаТокен = Неопределено Тогда
						СтрокаТокен = НастройкаОбъект.Параметры.Добавить();
						СтрокаТокен.Имя = "Токен";
					КонецЕсли;
					СтрокаТокен.Значение = Токен;
					
					СтрокаДатаДействияТокена = НастройкаОбъект.Параметры.Найти("ДатаДействияТокена", "Имя");
					Если СтрокаДатаДействияТокена = Неопределено Тогда
						СтрокаДатаДействияТокена = НастройкаОбъект.Параметры.Добавить();
						СтрокаДатаДействияТокена.Имя = "ДатаДействияТокена";
					КонецЕсли;
					СтрокаДатаДействияТокена.Значение = ДатаДействияТокена;
					
					СтрокаТипТокена = НастройкаОбъект.Параметры.Найти("ТипТокена", "Имя");
					Если СтрокаТипТокена = Неопределено Тогда
						СтрокаТипТокена = НастройкаОбъект.Параметры.Добавить();
						СтрокаТипТокена.Имя = "ТипТокена";
					КонецЕсли;
					СтрокаТипТокена.Значение = ТипТокена;
			...
			КонецПопытки;
		Иначе
			ВызватьИсключение "Не удалось получить token через keycloak";
		КонецЕсли;
	КонецЕсли;
	
	Возврат СтрШаблон("%1 %2", ТРег(ТипТокена), Токен);
...
Функция ОтправитьСообщениеВОчередьAMQP(Обменник, routing_key, ТелоСообщения, ПрерыватьПриОшибке = Истина, Таймаут = 10,
	...
	ОтправлятьЧерезНовыйДЭК = КодОбщегоНазначения.ПроверитьВключенаЛиДополнительнаяНастройкаПрограммы(
		"Отправлять сообщения AMQP через новый ДЭК");
...
Процедура ПолучитьМетодОбменаСообщениями(ИмяСервера, АдресРесурса, ТекущийМетод = "",
	...
	Попытка
		...
		Если КодОбщегоНазначенияКлиентСерверПовтИсп.ЭтоРабочаяБаза()
		...
		ИначеЕсли ЗначениеЗаполнено(ФайлСертификатКлиента) Тогда
			...
			// TODO: Добавить модуль работа с файлами и раскоментить строку ниже 
			//ддСертификатСервера = РаботаСФайлами.ДвоичныеДанныеФайла(ФайлСертификатКлиента, Ложь);
			ддСертификатСервера = Новый ДвоичныеДанные(ИмяВремФайлаСертификатаКлиента);
