...
Функция ПолучитьДополнительнуюНастройкуПрограммы(ИмяПредопределенного = "", Код = "", Наименование = "") Экспорт
	...
	Возврат КодОбщегоНазначенияПовтИсп.ПолучитьДополнительнуюНастройкуПрограммы(ИмяПредопределенного, Код, Наименование);
...
Функция ПолучитьРезультатСКД(АдресВхСхемыСКД, ТипРезультата, СтруктураПараметров = Неопределено,
	знач НастройкиСКД = Неопределено, НаборыДанных = Неопределено) Экспорт
...
Функция СоздатьПриНеобходимостиНаборДопРеквизитовИСведений(ИмяПредопределенногоНабора, Наименование) Экспорт
	
	Набор = КодОбщегоНазначения.НайтиНаборДополнительныхРеквизитовИСведений(ИмяПредопределенногоНабора);
	Если Не ЗначениеЗаполнено(Набор) Тогда
		Набор = Справочники.НаборыДополнительныхРеквизитовИСведений.СоздатьЭлемент();
		Набор.ИмяПредопределенногоНабора = ИмяПредопределенногоНабора;
		Набор.Наименование = Наименование;
		Набор.Используется = Истина;
		Набор.Записать();
	Иначе
		Набор = Набор.ПолучитьОбъект();
	КонецЕсли;
...
Функция СоздатьПриНеобходимостиДополнительнуюНастройкуПрограммы(Наименование, Включена = Ложь, СтруктураДопНастроек = Неопределено) Экспорт
	
	Настройка = КодОбщегоНазначения.ПолучитьДополнительнуюНастройкуПрограммы(,, Наименование);
	Если Настройка = Неопределено Тогда
		Настройка = Справочники.ДополнительныеНастройкиПрограммы.СоздатьЭлемент();
		Настройка.Наименование = Наименование;
		Настройка.Включена = Включена;
		
		Если Не СтруктураДопНастроек = Неопределено Тогда
			Для Каждого ЭлементНастройки Из СтруктураДопНастроек Цикл
				Если ТипЗнч(ЭлементНастройки.Значение) = Тип("Массив") Тогда
					Для Каждого ЭлементМассива Из ЭлементНастройки.Значение Цикл
						НовСтр = Настройка.Параметры.Добавить();
						НовСтр.Имя = ЭлементНастройки.Ключ;
						НовСтр.Значение = ЭлементМассива;
					КонецЦикла;
				Иначе
					НовСтр = Настройка.Параметры.Добавить();
					НовСтр.Имя = ЭлементНастройки.Ключ;
					НовСтр.Значение = ЭлементНастройки.Значение;
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
...
// Функция создает новый элемент справочника и привязывает его к предопределенному элементу
Функция СоздатьПредопределенныйЭлементСправочника(ИмяСправочника, ИмяПредопределенныхДанных, НаименованиеНовогоЭлемента, ЭтоГруппа = Ложь
													, СтруктураДопСвойств = Неопределено, УИД = Неопределено) Экспорт
	...
	ИсточникНоменклатура = ОператорЗапроса.Источники.Добавить("Справочник." + ИмяСправочника, "ВыбранныйСправочник");
	
	ОператорЗапроса.ВыбираемыеПоля.Добавить("ВыбранныйСправочник.Ссылка");
	ОператорЗапроса.Отбор.Добавить("ВыбранныйСправочник.ИмяПредопределенныхДанных = &ИмяПредопределенныхДанных");
	...
	Если РезультатЗапроса.Пустой() Тогда
		...
		ТекстСообщения = " - Создан новый элемент Справочника ""%1"", связан с предопредленным значением ""%2"".";
		ТекстОшибки = " - Ошибка создания нового элемента Справочника ""%1"" для привязки к предопредленному значению ""%2""!"
						+ " (Описание ошибки: %3)";
	Иначе
		...
		ТекстСообщения = " - Обновлен элемент Справочника ""%1"", связаный с предопредленным значением ""%2"".";
		ТекстОшибки = " - Ошибка обновления элемента Справочника ""%1"", связанного с предопредленным значением ""%2""!"
						+ " (Описание ошибки: %3)";
	КонецЕсли;
	...
	Попытка
		...
		Если НЕ УИД = Неопределено Тогда
			...
			СсылкаПоУИД = Справочники[ИмяСправочника].ПолучитьСсылку(Новый УникальныйИдентификатор(УИД));
			Если НЕ Элемент.Ссылка.Пустая() И НЕ СсылкаПоУИД = Элемент.Ссылка Тогда
				Элемент.ИмяПредопределенныхДанных = "";
				ТекстСообщения = " - Сменился элемент Справочника ""%1"", связаный с предопредленным значением ""%2"".";
				ТекстОшибки = " - Ошибка смены предопределенного элемента ""%2"" Справочника ""%1""!"
								+ " (Описание ошибки: %3)";
				Элемент.Записать();
			КонецЕсли;
			
			ОбъектПоУИД = СсылкаПоУИД.ПолучитьОбъект();
			Если ОбъектПоУИД = Неопределено Тогда
				...
				Элемент.УстановитьСсылкуНового(СсылкаПоУИД);
		...
		Элемент.Наименование = НаименованиеНовогоЭлемента;
		Элемент.ИмяПредопределенныхДанных = ИмяПредопределенныхДанных;
		Если СтруктураДопСвойств <> Неопределено Тогда
			ЗаполнитьЗначенияСвойств(Элемент, СтруктураДопСвойств);
		КонецЕсли;
		
		Элемент.Записать();
		
		ТекстСообщения = СтрШаблон(ТекстСообщения, ИмяСправочника, ИмяПредопределенныхДанных);
		ВидОповещения = СтатусСообщения.Информация;
		
		Результат = Элемент.Ссылка;
		
	Исключение
		ТекстСообщения = СтрШаблон(ТекстОшибки, ИмяСправочника, ИмяПредопределенныхДанных, ОписаниеОшибки());
		ВидОповещения = СтатусСообщения.ОченьВажное;
	КонецПопытки;
	
	Сообщить(ТекстСообщения, ВидОповещения);
	
	Возврат Результат;
	
КонецФункции

// Функция создает новый предопределенный элемент плана видов характеристик
//
// Параметры
//  ОсновныеДанные - Структура - Значения основных данных
//                 ИмяПВХ - Строка - Имя плана видов характеристик в котором создается элемент
//                 ИмяПредопределенныхДанных - Строка - Идентификатор предопределенного элемента
//                 Наименование - Строка - Наименование создаваемого элемента
//                 ТипЗначения - ОписаниеТипов - Тип значения создаваемого элемента
//                 ЭтоГруппа - Булево - Признак создания группы (если не указано или Ложь создается элемент)
//  ДополнительныеДанные - Структура - Значения остальных реквизитов создаваемого элемента
//
// Возвращаемое значение:
//   Булево - Результат создания / обновления элемента (Истина - успешно, Ложь - ошибка)
//
Функция СоздатьПредопределенныйЭлементПланаВидовХарактеристик(ОсновныеДанные, ДополнительныеДанные = Неопределено)
	
	// Проверка наличия обязательных значений в ОсновныеДанные
	ОбязательныеЗначения = Новый Массив;
	ОбязательныеЗначения.Добавить("ИмяПВХ");
	ОбязательныеЗначения.Добавить("ИмяПредопределенныхДанных");
	ОбязательныеЗначения.Добавить("Наименование");
	ОбязательныеЗначения.Добавить("ТипЗначения");
	
	Для Каждого ОбязательноеЗначение Из ОбязательныеЗначения Цикл
		
		Если Не ОсновныеДанные.Свойство(ОбязательноеЗначение)
			Или Не ЗначениеЗаполнено(ОсновныеДанные[ОбязательноеЗначение]) Тогда
	...
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ИмяПредопределенныхДанных", ОсновныеДанные.ИмяПредопределенныхДанных);
	
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ПВХ.Ссылка КАК Ссылка
	|ИЗ
	|	ПланВидовХарактеристик.ИмяПВХ КАК ПВХ
	|ГДЕ
	|	ПВХ.ИмяПредопределенныхДанных = &ИмяПредопределенныхДанных";
	
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "ИмяПВХ", ОсновныеДанные.ИмяПВХ);
	...
	Если РезультатЗапроса.Пустой() Тогда
		...
		Если ОсновныеДанные.Свойство("ЭтоГруппа") И ОсновныеДанные.ЭтоГруппа Тогда
			ОбъектПВХ = ПланыВидовХарактеристик[ОсновныеДанные.ИмяПВХ].СоздатьГруппу();
		Иначе 
			ОбъектПВХ = ПланыВидовХарактеристик[ОсновныеДанные.ИмяПВХ].СоздатьЭлемент();
		КонецЕсли;
		
		ОбъектПВХ.ИмяПредопределенныхДанных = ОсновныеДанные.ИмяПредопределенныхДанных;
		
		ТекстСообщения = СтрШаблон(" - Создан новый элемент ПВХ %1 ""%2"", связан с предопредленным значением ""%3"".",
			ОсновныеДанные.ИмяПВХ, ОсновныеДанные.Наименование, ОсновныеДанные.ИмяПредопределенныхДанных);
			
		ТекстОшибки = СтрШаблон(" - Ошибка создания нового элемента ПВХ %1 ""%2"" для привязки к предопредленному значению ""%3""! (Описание ошибки: %4)",
			ОсновныеДанные.ИмяПВХ, ОсновныеДанные.Наименование, ОсновныеДанные.ИмяПредопределенныхДанных, ОписаниеОшибки());
	...
	Иначе
		...
		ТекстСообщения = СтрШаблон(" - Обновлен элемент ПВХ %1 связаный с предопредленным значением ""%2"".",
			ОсновныеДанные.ИмяПВХ, ОсновныеДанные.ИмяПредопределенныхДанных);
			
		ТекстОшибки = СтрШаблон(" - Ошибка обновления элемента ПВХ %1 связанного с предопредленным значением ""%2""! (Описание ошибки: %3)",
			ОсновныеДанные.ИмяПВХ, ОсновныеДанные.ИмяПредопределенныхДанных, ОписаниеОшибки());
	
	КонецЕсли;
	
	ОбъектПВХ.Наименование = ОсновныеДанные.Наименование;
	ОбъектПВХ.ТипЗначения = ОсновныеДанные.ТипЗначения;
	
	Если ДополнительныеДанные <> Неопределено Тогда
		ЗаполнитьЗначенияСвойств(ОбъектПВХ, ДополнительныеДанные);
	КонецЕсли;
...
// Процедура - выполняет привязку предопределенного значения к элементу справочника,
// Внимание!!! поиск элемента справочника осуществляется по коду,
// удостоверьтесь что передаваемый код уникален.
//
// Параметры:
//			ИмяСправочника - Строка - имя справочника к элементу которого необходимо привязать предопределенный,
//			КодЭлемента - Строка - код элемента справочника к которому необходимо привязать предопределенный,
//          ИмяПредопределенныхДанных - Строка - идентификатор предопределенного значения которое необходимо привязать.
//
Процедура ПривязатьПредопределенноеЗначениеКЭлементуСправочникаПоКоду(ИмяСправочника, КодЭлемента, ИмяПредопределенныхДанных)
	
	Попытка
		
		НайденныйЭлемент = Справочники[ИмяСправочника].НайтиПоКоду(КодЭлемента);
		
		Если ЗначениеЗаполнено(НайденныйЭлемент) Тогда 
			
			ОбъектЭлементаСправочника = НайденныйЭлемент.ПолучитьОбъект();
			
			Если ОбъектЭлементаСправочника.ИмяПредопределенныхДанных = ИмяПредопределенныхДанных Тогда
				Сообщить(" - Элемент справочника """ + ИмяСправочника + """"
					   + " код элемента """ + КодЭлемента + """"
					   + " уже связан с предопредленным значением """ + ИмяПредопределенныхДанных + """.", СтатусСообщения.Информация);	
			Иначе
				ОбъектЭлементаСправочника.ИмяПредопределенныхДанных = ИмяПредопределенныхДанных;
				ОбъектЭлементаСправочника.Записать();
				Сообщить(" - Элемент справочника """ + ИмяСправочника + """"
					   + " код элемента """ + КодЭлемента + """"
					   + " связан с предопредленным значением """ + ИмяПредопределенныхДанных + """.", СтатусСообщения.Информация);
			 КонецЕсли;
				 
		Иначе
			Сообщить(" - Не найден элемент справочника """ + ИмяСправочника + """"
				   + " код элемента """ + КодЭлемента + """"
				   + " для связывания с предопредленным значением """ + ИмяПредопределенныхДанных + """!", СтатусСообщения.ОченьВажное);
		КонецЕсли;
		
	Исключение
		
		Сообщить(" - Ошибка связывания элемента справочника """ + ИмяСправочника + """"
			   + " код элемента """ + КодЭлемента + """"
			   + " с предопредленным значением """ + ИмяПредопределенныхДанных + """!"
			   + " (Описание ошибки: " + ОписаниеОшибки() + ")", СтатусСообщения.ОченьВажное);
